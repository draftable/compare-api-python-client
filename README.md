...
This is a thin Python 2/3 client for Draftable's [document comparison API](https://draftable.com/comparison-api). It wraps the available endpoints, and handles authentication and signing for you. The library is [available on PyPI](https://pypi.python.org/pypi/draftable-compare-api) as `draftable-compare-api`.
...
`Client(account_id: str, auth_token: str)` will construct an API client. At present, `Client` has a single property, `comparisons`, that yields a `ComparisonsEndpoint` that manages the comparisons for your API account.
